...
The Expo + React Native app for Neario, a hyperlocal marketplace for home-based sellers and local commerce in Tier-2 Indian cities.

`apps/mobile` inside the Neario pnpm workspace. Backend: `apps/api`. Shared zod schemas: `packages/shared`.
...
pnpm --filter @neario/shared build      # build shared zod/types first
...
Tokens (`src/theme/`) + motion (`src/motion/`) are the source of truth — see **"Neario — Design System v1.2"** in the Library. **Components never hardcode color / spacing / radii**; everything reads from `useTheme()`. That's what lets light + dark ship from one component.
